...
describe('DownloadProgressTracker', () => {
  it('maps a single stream 0-100 to 0-90 overall', () => {
    ...
    expect(tracker.onLine('download:CROPCORN|  0.0%|0|100')).toBeCloseTo(0);
    expect(tracker.onLine('download:CROPCORN| 50.0%|50|100')).toBeCloseTo(45);
    expect(tracker.onLine('download:CROPCORN|100.0%|100|100')).toBeCloseTo(90);
  });

  it('detects a second stream (percent reset) and maps it to 90-98', () => {
    const tracker = new DownloadProgressTracker();
    tracker.onLine('download:CROPCORN|100.0%|100|100');
    expect(tracker.onLine('download:CROPCORN|  0.0%|0|50')).toBeCloseTo(90);
    expect(tracker.onLine('download:CROPCORN| 50.0%|25|50')).toBeCloseTo(94);
    expect(tracker.onLine('download:CROPCORN|100.0%|50|50')).toBeCloseTo(98);
  });

  it('ignores non-progress lines and never goes backwards within a phase', () => {
    const tracker = new DownloadProgressTracker();
    tracker.onLine('download:CROPCORN| 60.0%|60|100');
    expect(tracker.onLine('[info] something')).toBeNull();
    expect(tracker.onLine('download:CROPCORN| 55.0%|55|100')).toBeCloseTo(54);
  });
});
